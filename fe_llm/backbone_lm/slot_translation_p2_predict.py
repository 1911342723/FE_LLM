"""Generate A/B/C prediction JSONL for FE-LLM P2 residual-adapter experiments."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate FE-LLM P2 A/B/C prediction JSONL.")
    ...
    parser.add_argument("--control-mode", choices=["random", "mismatch"], default="random")
...
def print_dry_run(args: argparse.Namespace) -> None:
    print("[p2-predict] dry-run：未下载模型，未生成预测。")
    print(f"[p2-predict] model_name = {args.model_name}")
    print(f"[p2-predict] ckpt_path = {args.ckpt_path}")
    print(f"[p2-predict] pred_path = {args.pred_path}")
    print("[p2-predict] 真正生成请显式追加 --run。")
...
def generate_one(
    ...
    device,
    intent_override: IntentState | None = None,
    ...
    for _ in range(args.max_new):
        ...
        if group in {"B", "C"}:
            ...
            adapted, _ = residual_adapter(hidden, intent, gamma=args.gamma)
            logits = lm_head(adapted)[0]
            if args.top_k > 0:
                top_ids = torch.topk(base_logits, k=min(args.top_k, base_logits.numel())).indices
                masked = torch.full_like(logits, -1e9)
                masked[top_ids] = logits[top_ids]
                logits = masked
    ...
    with open(args.pred_path, "w", encoding="utf-8") as f:
        for idx, pair in enumerate(pairs):
            for group in ("A", "B", "C"):
                ...
                count += 1
    print(f"[p2-predict] wrote {count} rows -> {args.pred_path}")
    return count
