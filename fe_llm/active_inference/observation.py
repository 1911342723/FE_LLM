...
# 统一 taxonomy 单一真相源：关键词领域的必需槽位由它派生（贯通 感知/NLU/教师 三层）。
...
# 槽位值表（城市）：用于"槽位级 belief"——检测订票类请求需要的 route 槽位是否已提供。
# 这是规则级特征（perception 本就规则化），供 policy/belief 做 answer vs ask_clarification 决策。
CITY_SLOT_VALUES = (
    "北京", "上海", "广州", "深圳", "杭州", "成都", "武汉", "西安",
    "南京", "重庆", "天津", "苏州", "长沙", "郑州", "青岛", "厦门",
)
# 订票类请求需要 route(出发地→目的地)槽位；刻意用订票域而非天气域，
# 避免与既有 weather→retrieve 的动作语义冲突（见 经验.md / 任务.md 决策点）。
BOOKING_TERMS = ("订票", "帮我订", "订一下", "预订", "买票", "订张", "订机票", "订火车票", "定票", "订一张")
# 订酒店类请求需要 city + date 两个槽位（多槽位场景）：全部凑齐才回答。
HOTEL_TERMS = ("订酒店", "订房", "订个酒店", "订间房", "订旅馆", "订宾馆", "订民宿")
# 提醒类请求需要 time 槽位（再增一个槽位领域，验证框架通用性）。
REMINDER_TERMS = ("提醒我", "设个提醒", "设置提醒", "定个闹钟", "定闹钟", "提醒一下", "加个提醒")
DATE_SLOT_TERMS = ("今天", "明天", "后天", "大后天", "周末", "周六", "周日", "本周末", "这周末", "下周末", "下周", "元旦", "春节")
...
def _detect_route_slot(text: str) -> str | None:
    """检测 route 槽位值（出发地→目的地），用于订票类请求的槽位级 belief 填充。"""
    cities = "|".join(CITY_SLOT_VALUES)
    match = re.search(rf"({cities})\s*(?:到|去|至|飞往|飞)\s*({cities})", text)
    if match:
        return f"{match.group(1)}到{match.group(2)}"
    return None


def _detect_date_slot(text: str) -> str | None:
    """检测 date 槽位值（用于订酒店等多槽位请求）。"""
    for term in DATE_SLOT_TERMS:
        if term in text:
            return term
    match = re.search(r"\d{1,2}\s*(?:月|号|日)", text)
    return match.group(0) if match else None


def _detect_time_slot(text: str) -> str | None:
    """检测 time 槽位值（用于提醒类请求）：时钟表达优先，回退到日期词。"""
    match = re.search(r"\d{1,2}\s*[:：]\s*\d{2}|\d{1,2}\s*点(?:\d{1,2}分?)?|早上|上午|中午|下午|晚上|凌晨", text)
    if match:
        return match.group(0)
    return _detect_date_slot(text)
...
# 可选的学习式 NLU 增强（依赖注入，默认关闭：感知层零额外依赖、既有行为不变）。
# 仅当关键词未命中任何意图、且学习式置信达标时，用它补充 required_slots（脱离关键词表）。
_LEARNED_NLU = None
_NLU_CONF_THRESHOLD = 0.6


def set_learned_nlu(nlu, conf_threshold: float = 0.6) -> None:
    """注入学习式 NLU（SlotIntentNLU），传 None 关闭。生产可在启动时调用一次。"""
    global _LEARNED_NLU, _NLU_CONF_THRESHOLD
    _LEARNED_NLU = nlu
    _NLU_CONF_THRESHOLD = conf_threshold
...
def extract_prompt_features(text: str) -> dict[str, Any]:
    ...
    # 槽位级特征（支持单槽位订票=route 与多槽位订酒店=city+date）；避开天气域保持 weather→retrieve。
    route_value = _detect_route_slot(stripped)
    has_booking = _contains_any(stripped, BOOKING_TERMS)
    has_hotel = _contains_any(stripped, HOTEL_TERMS)
    has_reminder = _contains_any(stripped, REMINDER_TERMS)
    date_value = _detect_date_slot(stripped)
    time_value = _detect_time_slot(stripped)
    provided_slots: dict[str, str] = {}
    if route_value is not None:
        provided_slots["route"] = route_value
    if has_hotel:
        hotel_city = next((c for c in CITY_SLOT_VALUES if c in stripped), None)
        if hotel_city:
            provided_slots["city"] = hotel_city
        if date_value:
            provided_slots["date"] = date_value
    elif has_reminder:
        if time_value:
            provided_slots["time"] = time_value
    else:
        # 纯槽位值轮次（为待填请求补信息）：裸城市 / 裸日期时间
        if stripped in CITY_SLOT_VALUES:
            provided_slots["city"] = stripped
        is_bare_datetime = (
            time_value is not None and len(stripped) <= 8
            and not has_question and not _contains_any(stripped, REQUEST_TERMS)
        )
        if is_bare_datetime:
            if date_value is not None:
                provided_slots["date"] = date_value
            provided_slots["time"] = time_value
    # keyword_domain：仅由关键词可靠检出的任务领域（booking/hotel/reminder/None）。
    # 它是 B2d 活跃领域(active_domain)的唯一来源——学习式 NLU 对裸槽位值的高置信猜测
    # （如"北京"→booking、"明天"→reminder）不可靠，绝不让它更新活跃领域，否则会污染语境。
    if has_reminder:
        keyword_domain = "reminder"
    elif has_hotel:
        keyword_domain = "hotel"
    elif has_booking:
        keyword_domain = "booking"
    else:
        keyword_domain = None
    if keyword_domain is not None:
        # 关键词领域的必需槽位派生自统一 taxonomy（单一真相源，值与历史一致）。
        required_slots = list(LEGACY_REQUIRED_SLOTS[keyword_domain])
    else:
        # 关键词未命中：仅在「无其它已识别信号」(非天气/记忆/安全/寒暄/逻辑冲突) 且学习式
        # 高置信时才补意图，避免误改 weather→retrieve / 记忆 / 拒答 / 寒暄 等既有行为。
        required_slots = []
        if _LEARNED_NLU is not None:
            other_signal = (
                needs_external
                or has_conflict
                or _has_memory_request(stripped, has_question)
                or _contains_any(stripped, UNSAFE_TERMS)
                or stripped in {"你好", "您好", "嗨", "在吗", "早上好", "晚上好", "谢谢", "谢谢你", "太感谢了", "麻烦你了"}
            )
            if not other_signal:
                intent, conf = _LEARNED_NLU.predict_conf(stripped)
                if intent != "none" and conf >= _NLU_CONF_THRESHOLD:
                    from fe_llm.active_inference.nlu.slot_intent_nlu import INTENT_REQUIRED_SLOTS
                    required_slots = list(INTENT_REQUIRED_SLOTS[intent])
                    # 注意：NLU 推断只补 required_slots，不设 keyword_domain（不更新活跃领域）。
    missing_here = [slot for slot in required_slots if slot not in provided_slots]
    first_provided = next(iter(provided_slots.items()), None)
    return {
        "length": len(stripped),
        ...
        # 槽位级 belief 用：provided_slots/required_slots 支持多槽位；其余为单值派生（向后兼容）。
        "provided_slots": provided_slots,
        "required_slots": required_slots,
        "keyword_domain": keyword_domain,  # B2d：仅关键词检出的可靠领域（活跃领域来源；无则 None）
        "provides_slot": first_provided[0] if first_provided else None,
        "slot_value": first_provided[1] if first_provided else None,
        # 裸槽位值轮 = 给了槽位值且无任何领域关键词（含 reminder）；B2d 据此判定"领域未明示跟进句"。
        "is_bare_slot_value": bool(provided_slots) and not has_booking and not has_hotel and not has_reminder,
        "requires_slot": missing_here[0] if missing_here else None,
    }
