...
## 6.5 实验 Real — 真实库 API 知识：整套机制非玩具（堵"合成任务"质疑）

把**真实 Python 库的标志性 API 惯用法**当作不同知识（每库一个 LoRA-ISO 块，变量名作可变实例验证泛化，动态选 base 不会的 4 个：numpy / pandas / torch / requests）：

| 验证项 | 结果 |
|---|---|
| 隔离不忘（顺序学 4 库，各用正确块） | 平均 acc **100%** |
| 可定位编辑（卸载某库块） | 平均特异性 **+1.00**（目标降 100%、旁观降 0%） |
| 按 import 路由 | import 触发词 **100%**（真实可靠）/ 能量 44% |

**结论**：整套机制（隔离不忘 + 可定位编辑 + 路由）在**真实库 API 知识**上成立——不只合成玩具，且结果比合成任务更干净（满分）。代码里 `import` 即天然、可靠的路由信号；纯能量学习化路由仍弱（开放问题）。
报告：`reports/code_real_library.md`；图：`reports/figs/code_real_library.png`。
...
python -m fe_llm.energy_lm.evaluation.code_real_library_eval       # Real 真实库API知识(非玩具)
```
