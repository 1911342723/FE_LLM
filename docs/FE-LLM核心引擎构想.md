...
## 29. 开放关系 NLU：免"记住"标记的关系绑定，拓宽活文本多跳覆盖（2026-06-16）

第 25 节活文本多跳的关系绑定要"记住X的R是Y"标记式（裸"X的R是Y"不触发，怕误伤）。本轮在
`MultiHopBindingNLU` 加一条**高精度开放关系绑定**：免"记住"也能绑定关系事实「X的R是Y」，但用窄触发守住
精度（仿 B2e/学习式 NLU 教训）。规则 `_OPEN_REL_BIND`：① key 必须"X的R"（**恰 1 个 的**=原子关系边，
2~12 字）→ 排除"今天是周一"这类无 的 copula；② value 须**实体化**（1~10 字、不含 的/是/标点、不以语气词
结尾）→ 排除"…是好的/对的/充足的"形容词谓语；③ 查询词收尾（…是多少/谁/什么）由查询模式先行截获。

精度实测（单测锁定）：

| 类别 | 样例 | 判定 |
|---|---|---|
| 关系事实（应绑） | A的经理是张三 / 项目甲的负责人是李四 / 服务器的IP是10.0.0.7 / 他的爱好是打球 | ✅ bind |
| copula（不绑） | 今天是周一（key 无 的） | ✅ none |
| 形容词谓语（不绑） | 这个是错的 / 今天的心情是好的 / 项目的预算是充足的 / 我觉得这样是对的（value 含 的） | ✅ none |
| 寒暄/任务（不绑） | 你好 / 我有点累 / 谢谢你 / 帮我订票 | ✅ none |
| 查询（判 query 非 bind） | A的经理是多少 / A的经理的工位是多少 | ✅ query |
| 复合（不开放绑，需"记住"） | A的项目的经理是张三（2 个 的） | ✅ none |

端到端（活文本多跳，免"记住"）：「项目甲的经理是张三 / 张三的工位是B302 → 项目甲的经理的工位是多少 →
答 B302（CoT trace 张三→B302）」，且"今天的心情是好的"不被劫持。新增 5 测试（NLU 精度 4 + 活文本免标记
端到端 1），全量 **198 测试全绿**（193→198，零回归）。

**意义**：活文本多跳从"必须用记住标记"放宽到"自然陈述关系事实即可绑定"，覆盖面更广，同时靠"恰 1 个 的 +
实体化 value + 查询优先"三重窄触发守住精度、不劫持寒暄/形容词谓语。诚实边界：仍是规则式高精度（覆盖
"X的R是Y"标记结构，非全开放实体/指代抽取）；复合所有格陈述（多个 的）仍需"记住"显式标记。

## 30. 检索之外的推理基元（比较/计数）→ 诚实负结果：未解锁新推理，价值仍锚定内容寻址取回（2026-06-16）

CAPCW 已证擅长内容寻址**取回/绑定**。本轮问：它能否扩到**对绑定值做运算**的推理——in-context **比较**
（取两值比大小）与**计数**（阈值上计数）？脚本 `world_model/capcw_reasoning_primitives_eval.py`（唯一变量=
world 结构 flat 单向量 vs CAPCW slot 工作空间，同 d=32 / 同 pair 编码器 / 同预算；2 任务、扫 K、2 seed）。

| 任务 | K | flat | CAPCW |
|---|---:|---:|---:|
| compare（取两值比大小，随机 0.5） | 3 | 0.920 | 0.910 |
|  | 5 | 0.868 | 0.872 |
|  | 7 | 0.833 | **0.963** |
| count（阈值上计数，随机≈1/(K+1)） | 3 | **1.000** | **1.000** |
|  | 5 | **1.000** | **1.000** |
|  | 7 | **1.000** | **1.000** |

**诚实负结果（无新增推理基元）**：

- **compare 优势是容量依赖的**：低 K（3/5）单向量够用、与 CAPCW 打平；仅高 K=7（容量受限）CAPCW +0.130，
  多 K 均值 +0.067 < 0.10（被低 K 平局拉低）。这是与**检索/绑定同源的容量效应**（§9 同款），**不是**新增的
  推理超能力——compare 本质就是"取回两值再比"，差异只在能否取回，不在"会不会比较"。
- **count 无 headroom**：flat 自身就 **1.000**——单向量用"值编码求和"（thermometer 式编码 + 池化）即可计数，
  **keys 完全无关**，根本不测内容寻址。这是又一例"任务瓶颈不在被测机制上"（与翻译被底座填满、动作分类被
  单向量吃满同类）——count 不是 CAPCW 的有效试金石。

**结论（边界定义）**：CAPCW 的价值仍锚定在**容量受限的内容寻址取回/绑定/链式组合**；它**未额外解锁**"比较/
计数"这类对值的运算推理（compare 优势=容量效应、count 无 headroom）。这是对 CAPCW 能力边界的诚实定界：
它是**内容寻址工作记忆引擎**（取回/绑定/induction/多跳链式），不是通用算术/聚合推理器。教训（再次）：测一个
机制"会不会新本事"前，先确认**任务瓶颈正好落在该机制上**——count 一上来 flat 就满分，说明瓶颈不在内容寻址。
报告 `docs/reports/capcw_reasoning_primitives_eval.{json,md}`；纯 --run 研究脚本，不入回归（全量仍 198 全绿）。

## 31. 直面"连连看"质疑：in-context 规则归纳 → 它不止查表，会规则外推（2026-06-16，正面意外）

§30 把 CAPCW 定界为"内容寻址取回/绑定引擎"，自然引出质疑：**它是不是只会"查已见的关联"（连连看/lookup）、
根本没在"推理"？** 直面这个问题——脚本 `world_model/capcw_rule_induction_eval.py`：in-context 规则归纳（规则=
循环移位，每例随机 shift k），给 K 个示例对 (x_i,(x_i+k)%n)，查询一个符号预测 (x_q+k)%n；查询分 **SEEN**
（在示例里，查表可解=连连看）与 **UNSEEN**（没出现，必须**归纳出 k 再应用**=超连连看）。关键：UNSEEN 的答案
**不在示例值里**（+k 是双射，x_q≠x_i ⇒ 答案≠任何示例值），无法靠拷贝，必须真外推。
...
（随机 0.05）**UNSEEN 0.97 ≫ 随机 → PASS(超连连看)**：engine **从示例归纳出移位规则、应用到没见过的输入**，
这是真正的**泛化/规则外推**，不是查表。**"CAPCW 像连连看"的直觉被证伪**——它不止链接已见关联。

**两条诚实洞察（与 §30 合起来看，画出完整图景）**：
- **"连连看"只是它的一个面**：内容寻址取回（SEEN 0.99，§9/§30 的容量优势）是一面；但整个 engine（pair 编码器
  + 工作空间 + readout）**还会 in-context 规则归纳并外推到未见输入**（UNSEEN 0.97）——所以它**不是单纯查表**。
- **规则归纳主要来自 readout、不专属内容寻址**：flat 单向量也能做（UNSEEN 0.934），CAPCW 仅略高（0.973）。
  即"智能"是分布式的：**内容寻址负责"取回/绑定"，readout 负责"计算/归纳/外推"**——两者都在，缺一就只剩半边。
  这也和 §30 自洽：内容寻址对"比较/计数/移位"这类**全局运算**不额外加分（readout 本就会），它的专长是
  **容量受限的按键取回**。

**结论（回应质疑）**：FE-LLM 这颗"脑子"**不是连连看那么笨**——它会查表（内容寻址），也会从几个例子里**归纳规则
并外推到新输入**（in-context rule learning，UNSEEN 0.97）。诚实边界：① 循环移位规则结构性强（可归纳），更
无结构的规则未必；② d=32 容量仍限制更复杂规则/更高负载；③ 规则归纳非 CAPCW 独有（readout 之功）。报告
`docs/reports/capcw_rule_induction_eval.{json,md}`；纯 --run 研究脚本，不入回归（全量仍 198 全绿）。

## 32. "是否裸增 d/数据就能到好效果？"→ 容量扩展曲线：裸增 d 不抬反降（2026-06-16，诚实意外）

直接测——脚本 `world_model/capcw_capacity_scaling_eval.py`：固定高负载 in-context 绑定（K=8），扫脑容量
d∈{16,32,64,128}，flat vs CAPCW，看绑定取回准确率随 d 的变化。
...
（n_vals=16，随机 0.0625）**裸增 d 不抬反降（意外）**：CAPCW 0.822→0.074，**最佳在最小的 d=16**，d≥64 训练
**塌缩/不稳定**——且 **不是欠训**（单独把 d=64 训到 epochs=160 / n_train=18000 仍 0.06）。flat 单向量在 K=8
下**任何 d 都≈随机**（均值池化装不下 8 个绑定，与 d 无关）。

**诊断**：PCWorkspace 的弛豫 dynamics（步长 α、precision、iters）是按小 d 调的；d 变大后重建误差/梯度量纲
变化使弛豫**发散**（与 §22"弛豫步过多会崩"同源，这次是"d 过大未重标定"）。这是**可修的归一化/超参问题**
（加 LayerNorm、按 d 缩放步长），不是深层不可能——但**修它=做标准可扩展架构该做的工程**。

**结论（回答"裸增 d/数据能否到好效果"）**：

- **不能裸增 d**：本引擎 d≥64 就塌（不稳定，非欠训），加数据/训练也不救。要受益于规模，须**架构工程**
  （归一化、弛豫稳定化、深度）——那本质是**重建一个标准可扩展 Transformer/LLM**，正是项目按**容量纪律
  刻意不走**的方向（不和大模型拼规模）。
- **即便修好，scale 只抬绝对精度、不改机制结论**：内容寻址优势恰在**小 d**（d=16 CAPCW 0.82≫flat、d=128
  反而最差）；多跳要 CoT(§26)、规则归纳靠 readout(§31)、内容寻址定位(§9/§30) 这些**机制真相与 d 无关**——
  它们才是项目的可迁移贡献（小规模便宜验证，结论对任意规模成立）。
- 一句话：**"裸增 d/数据→好效果"对本引擎不成立（先崩）；真要 scale 就是去做大模型工程（刻意非目标）。
  项目的价值是机制（便宜验证、规模无关），不是拼容量。** 诚实：collapse 大概率可由归一化修好，但那是另一条
  （大模型）路。报告 `docs/reports/capcw_capacity_scaling_eval.{json,md}`；纯 --run 研究脚本，不入回归（198 全绿）。

## 33. 从序列直接读关系：内容寻址不限于显式 pair（2026-06-16，更贴近真语言）

此前 CAPCW 的绑定都喂**显式 (key,value) 对**。本轮把关系以**扁平 token 序列**给出（[e1 r1 v1 e2 r2 v2 …]，更像
真语言的输入形态），模型须从序列读出 (实体·关系→值) 三元组，再按 (查询实体·关系) 取回值。脚本
`world_model/capcw_sequence_relation_eval.py`（唯一变量=world 结构 flat 单向量 vs CAPCW slot，同 d=32 / 同
序列前端 / 同预算）：
...
（随机 0.028）多 K(≥6) CAPCW−flat = **+0.564 → PASS**：关系以扁平序列给出时，slot 工作空间仍按 (实体·关系)
内容寻址取回值、显著胜单向量（低 K 单向量够用故打平，高 K 容量受限处 CAPCW 决定性胜——与 §9 容量依赖一脉
相承）。**结论：CAPCW 的内容寻址取回不限于"显式 pair"——能从更贴近真语言的扁平序列里读关系并按内容取回。**
报告 `docs/reports/capcw_sequence_relation_eval.{json,md}`；纯 --run 研究脚本，不入回归（198 全绿）。

## 34. 活文本工程加固：指代消解 + 有界工作记忆（2026-06-16）

把活文本多跳推到更自然/更鲁棒，两项工程加固（均加法式、单测锁定）：

- **指代消解（coreference）**：活文本里"他/她/它…的R"回指上文刚提到的实体。controller 按会话记最近实体
  （上一条绑定的 value=刚引入的实体），代词出现时替换。实测：「记住张三的经理是李四」→「**他**的工位是C302」
  （他=李四，自动绑成"李四的工位=C302"）→「张三的经理的工位是多少」→ 答 C302（trace 李四→C302）。即代词
  让**自然录入链**成立（引入实体后用代词补属性）。高精度有限代词集（他/她/它/其…），换话题 reset 清指代。
- **有界工作记忆（bounded working memory）**：链式工作记忆的字符串词表满（>n_sym）时**FIFO 淘汰最旧符号**
  （连其绑定、复用其 id），而非抛错——工作记忆本就该有界、忘最旧。实测：n_sym=8 绑 5 条边(10 符号)→最旧
  被忘、最近保留、查已忘→ASK 而不崩。这把"接回真实 controller"的多会话/长对话边界补成**优雅降级**。

新增 2 测试（指代消解端到端 + 有界淘汰），全量 **200 测试全绿**（198→200，零回归）。诚实边界：指代消解是
高精度规则（回指最近 value 实体，非全 coreference）；有界 WM 是 FIFO（非 LRU/重要度），够用即可。
